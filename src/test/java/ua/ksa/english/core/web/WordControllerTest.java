...
//        WordDTO request = WordDTO.builder().foreign("test").translate("тест").build();
//        String raw = json(request);
//        log.info("request {}", raw);
//        mockMvc.perform(MockMvcRequestBuilders.post(WordsController.WORDS_URL)
//                .content(raw)
//                .contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
//                .andDo(h -> log.info("response {}", h.getResponse().getContentAsString()))
//                .andExpect(status().isOk())
//                .andExpect(jsonPath("$.foreign").value(request.getForeign()))
...
//        WordDTO toSerialize = WordDTO.builder().foreign("test").translate("тест").build();
